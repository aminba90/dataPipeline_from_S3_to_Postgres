...
def calc_stage_dfs(df1: DataFrame, df2: DataFrame , columns: array):
    """
        A user-defined function to calculate cards view and articles view events and 
        merge them into a single dataframe.

        Parameters:
            df1 (pd.dataframe): provided dataframe from output of "card_views".
            df2 (pd.dataframe): provided dataframe from output of "article_views".
            columns (array): a series of columns that are used as a group by key in
                                        the calculations.
        Returns:
            finallDfTable (dataFrame): a ready to insert dataframe.
...
def prepare_main_df(df: DataFrame):
    """
        A user-defined function to clean and prepare input dataframe for the desired calculations.

        Parameters:
            df (pd.dataframe): provided dataframe from the output of read_data_from_s3 function .
        Returns:
            cardViewDf (dataFrame): cleaned and flattened dataframe for cardView.
            articleViewDf  (dataFrame): cleaned and flattened dataframe articleView.
    ...
    dfFromCSVJSON =  df.na.drop(subset=["ATTRIBUTES"]).select(col("TIMESTAMP"), col("EVENT_NAME"), col("MD5(USER_ID)").alias("user_id"), (from_json(col("ATTRIBUTES"), schema)).alias("ATTRIBUTES")).select("TIMESTAMP","USER_ID" ,"EVENT_NAME","ATTRIBUTES.*")
    finalDf = dfFromCSVJSON.select(col("id").alias("article_id"), col("publishTime"), col("title"), col("category"), col("EVENT_NAME"), col("TIMESTAMP"), col("user_id"))\
...
def load_to_db(df: DataFrame, targetTable: str, connection_uri:str):
    """
        A user-defined function to load input dataframe into postgres table.

        Parameters:
            df (dataframe): provided dataframe for inserting it into table.
            targetTable (String): name of a table.
            connection_uri (String): a uri for connecting to postgres DB.
    """
    df\
    .write\
    .format("jdbc")\
...
def read_data_from_s3(spark):
    ...
    df = spark.read.option("delimiter", "\t").option("header",True)\
        .csv("s3a://"+CONFIG["AWS_KEY"]+":"+CONFIG["AWS_SECRET"] +"@upday-data-assignment/lake/")
    return df
